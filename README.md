...
### 2. Fix Bundle Identifier (if needed)

The converter may generate mismatched bundle identifiers. In the generated `.xcodeproj`, ensure the app's `PRODUCT_BUNDLE_IDENTIFIER` is `com.local.regexfind` (all lowercase) so the extension's `com.local.regexfind.Extension` is properly prefixed.
...
- **No dynamic content tracking** — Highlights become stale if the page content changes after searching; re-search to refresh
